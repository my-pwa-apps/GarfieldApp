...
test('search, social and structured metadata consistently describe the canonical app', async () => {
  ...
  assert.ok(image.length < 600 * 1024);
  ...
  assert.deepEqual(schema.screenshot, manifest.screenshots.map(shot => new URL(shot.src, canonical).href));
  assert.doesNotMatch(files.get('serviceworker.js').toString(), /screenshots\//);
  assert.doesNotMatch(html, /<link[^>]*rel="preload"[^>]*screenshots\//);
